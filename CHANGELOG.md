...
- **Zu kleiner Abstand auf der IMAP-Konten- und der Benutzer-Seite**: Die Buttons
  „Neues Konto“ (`frontend/src/pages/AccountsPage.tsx`) und „Neuer Benutzer“
  (`frontend/src/pages/UsersPage.tsx`) hatten keinen `page-actions`-Wrapper und damit
  keinen `margin-bottom` zur darunterliegenden Überschrift. Fix analog zur
  Drucker-Seite: Buttons jeweils in `<div className="page-actions">` verpackt.
